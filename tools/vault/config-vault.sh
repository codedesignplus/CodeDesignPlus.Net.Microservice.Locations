...
vault kv put -mount=security-codedesignplus-keyvalue ms-locations \
...
vault kv get -mount=security-codedesignplus-keyvalue ms-locations

# Write database configuration
echo "$newlines"
echo "${BLUE}7. Writing database configuration...${NC}"
vault write security-codedesignplus-database/config/db-ms-locations \
    plugin_name=mongodb-database-plugin \
    allowed_roles="ms-locations-mongo-role" \
    connection_url="mongodb://{{username}}:{{password}}@mongo:27017/admin?ssl=false" \
    username="admin" \
    password="password"

vault write security-codedesignplus-database/roles/ms-locations-mongo-role \
    db_name=db-ms-locations \
    creation_statements='{ "db": "admin", "roles": [{ "role": "readWrite", "db": "db-ms-locations" }] }' \
    default_ttl="1h" \
    max_ttl="24h"

vault read security-codedesignplus-database/creds/ms-locations-mongo-role
...
vault write security-codedesignplus-rabbitmq/roles/ms-locations-rabbitmq-role \
    vhosts='{"/":{"write": ".*", "read": ".*", "configure": ".*"}}'

vault read security-codedesignplus-rabbitmq/creds/ms-locations-rabbitmq-role