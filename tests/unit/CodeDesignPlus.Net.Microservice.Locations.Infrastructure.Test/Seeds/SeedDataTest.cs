...
public class SeedDataTest
{
    ...
    public void NeighborhoodNames_HaveAccentsAndUppercaseRomanNumerals()
    {
        ...
        var unaccented = new HashSet<string>
        {
        ...
        var lowercaseRomans = new HashSet<string> { "Ii", "Iii", "Iv", "Vi", "Vii", "Ix" };

        var words = Neighborhoods.SelectMany(x => x.Name.Split(' ', StringSplitOptions.RemoveEmptyEntries)
            .Select(w => (x.Name, Word: w.Trim(',', '.', '(', ')', ':', '"'))));

        Assert.Empty(words.Where(p => unaccented.Contains(p.Word) || lowercaseRomans.Contains(p.Word)).Select(p => p.Name));
