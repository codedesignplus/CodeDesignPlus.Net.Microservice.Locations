...
) : DomainEvent(aggregateId, eventId, occurredAt, metadata)
{
    ...
    public static CountryDeletedDomainEvent Create(Guid aggregateId, string name, string alpha2, string alpha3, string code, string phoneCode, string? capital, Guid idCurrency, string timeZone, string nameNative, string region, string subRegion, double latitude, double longitude, string? flag, bool isActive)
    {
        return new CountryDeletedDomainEvent(aggregateId, name, alpha2, alpha3, code, phoneCode, capital, idCurrency, timeZone, nameNative, region, subRegion, latitude, longitude, flag, isActive);
    }
}
